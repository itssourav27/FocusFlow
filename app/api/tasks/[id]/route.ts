...
export async function PATCH(request: NextRequest, { params }: Params) {
  try {
    ...
    return NextResponse.json({ error: "Failed to update task", detail: String(error) }, { status: 500 });
  }
}
...
export async function DELETE(_request: NextRequest, { params }: Params) {
  try {
    ...
    return NextResponse.json({ error: "Failed to delete task", detail: String(error) }, { status: 500 });
  }
}
